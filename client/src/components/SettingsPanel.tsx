  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-start justify-center pt-[10vh]" onClick={onClose}>
      <div className="w-full max-w-md bg-white dark:bg-dark-secondary rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-surface-tertiary dark:border-dark-tertiary">
        ...
        <div className="p-5 space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => { const next = !isDark; document.documentElement.classList.toggle('dark', next); localStorage.setItem('kairus_dark', String(next)); }} className={`w-11 h-6 rounded-full transition-colors relative ${isDark ? 'bg-accent' : 'bg-surface-tertiary dark:bg-dark-tertiary'}`}>
          ...
          <p className="text-xs text-text-tertiary text-center pt-4 border-t border-surface-tertiary dark:border-dark-tertiary">Kairus v0.1.0</p>
        </div>
      </div>
    </div>
  );
