...
export default function ChatList({ chats, activeChat, onSelect, onNewChat, onSettings, onLogout, user, sidebarOpen, onToggleSidebar, showMobileList }: Props) {
  ...
  return (
    <div className={`${sidebarOpen ? 'w-80 min-w-[20rem]' : 'w-0 min-w-0'} ${showMobileList ? 'flex' : 'hidden md:flex'} flex-col border-r border-tertiary dark:border-dark-tertiary bg-surface dark:bg-dark-surface transition-all duration-200 overflow-hidden`}>
      <div className="h-14 flex items-center justify-between px-4 border-b border-tertiary dark:border-dark-tertiary shrink-0">
        ...
        <div className="relative">
          ...
          {menu && <div className="absolute right-0 top-full mt-1 w-48 bg-white dark:bg-dark-secondary rounded-xl shadow-lg border border-tertiary dark:border-dark-tertiary overflow-hidden z-50">
            <button onClick={() => { onSettings(); setMenu(false); }} className="w-full px-4 py-2.5 text-left hover:bg-tertiary dark:hover:bg-dark-tertiary text-sm">Settings</button>
            <button onClick={() => { onLogout(); setMenu(false); }} className="w-full px-4 py-2.5 text-left hover:bg-tertiary dark:hover:bg-dark-tertiary text-sm text-red-400">Sign out</button>
          </div>}
        </div>
      </div>
      <div className="p-2">
        <div className="relative">
          ...
          <input className="w-full pl-9 pr-3 py-2 rounded-xl bg-tertiary dark:bg-dark-tertiary text-sm placeholder-text-tertiary" placeholder="Search..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
      </div>
      <div className="flex-1 overflow-y-auto">
        {filtered.map(chat => (
          <button key={chat.id} onClick={() => onSelect(chat.id)} className={`w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-tertiary dark:hover:bg-dark-tertiary transition-colors ${activeChat === chat.id ? 'bg-accent-light dark:bg-accent/20' : ''}`}>
      ...
      <div className="p-3 border-t border-tertiary dark:border-dark-tertiary">
